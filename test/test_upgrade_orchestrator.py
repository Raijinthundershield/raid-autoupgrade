...
from autoraid.autoupgrade.autoupgrade import StopCountReason
...
class TestUpgradeOrchestrator:
    ...
    @patch("autoraid.services.upgrade_orchestrator.count_upgrade_fails")
    def test_orchestrator_count_workflow_calls_services(
        self,
        mock_count_upgrade_fails,
        ...
        # Setup count_upgrade_fails mock
        mock_count_upgrade_fails.return_value = (5, StopCountReason.MAX_FAILS)
        ...
        # Verify count_upgrade_fails was called
        assert mock_count_upgrade_fails.called

        # Verify result
        assert n_fails == 5
        assert reason == StopCountReason.MAX_FAILS
    ...
    def test_orchestrator_count_workflow_re_enables_network(
        ...
        # Setup count_upgrade_fails to raise exception
        mock_count_upgrade_fails.side_effect = RuntimeError("Test exception")
    ...
    def test_orchestrator_spend_workflow_calls_services(
        ...
        # Setup count_upgrade_fails mock (simulate upgrade after 3 fails)
        mock_count_upgrade_fails.return_value = (3, StopCountReason.UPGRADED)
