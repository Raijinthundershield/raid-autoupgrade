...
class TestCLIIntegration:
    ...
    def test_count_command_help_unchanged(self, baseline_dir):
        """Test that count command help output matches baseline (US4 acceptance criteria)."""
        ...
        current_help = result.stdout

        # Read baseline help output
        baseline_file = baseline_dir / "integration_baseline_count_help.txt"
        if baseline_file.exists():
            with open(baseline_file) as f:
                baseline_help = f.read()

            # Compare outputs (allowing for minor formatting differences)
            assert current_help == baseline_help, (
                "Count command help output has changed. "
                "This violates US4 (Unchanged User Workflows). "
                f"Expected:\n{baseline_help}\n\nGot:\n{current_help}"
            )
        else:
            # If baseline doesn't exist, save current output as baseline
            with open(baseline_file, "w") as f:
                f.write(current_help)
            pytest.skip(
                f"Baseline file created at {baseline_file}. Re-run tests to validate."
            )

    def test_spend_command_help_unchanged(self, baseline_dir):
        """Test that spend command help output matches baseline (US4 acceptance criteria)."""
        # Capture current help output
        result = subprocess.run(
            ["uv", "run", "autoraid", "upgrade", "spend", "--help"],
            capture_output=True,
            text=True,
            cwd=Path(__file__).parent.parent,
        )
        ...
        # Read baseline help output
        baseline_file = baseline_dir / "integration_baseline_spend_help.txt"
        if baseline_file.exists():
            with open(baseline_file) as f:
                baseline_help = f.read()
            ...
            # Compare outputs
            assert current_help == baseline_help, (
                "Spend command help output has changed. "
                "This violates US4 (Unchanged User Workflows). "
                f"Expected:\n{baseline_help}\n\nGot:\n{current_help}"
            )
        else:
