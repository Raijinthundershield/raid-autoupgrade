...
class TestUpgradeOrchestrator:
    ...
    def test_behavior_parity_with_fixture_images(self):
        """Test that new monitor API produces same results as old implementation.

        This test verifies backward compatibility by comparing the behavior
        of the new UpgradeAttemptMonitor with the old UpgradeStateMachine
        using the same fixture images.
        """
        import cv2
        ...
        # Load fixture images
        fixtures_dir = (
            Path(__file__).parent.parent / "fixtures" / "images" / "progress_bar_state"
        )
        fail_image = cv2.imread(str(fixtures_dir / "fail.png"))
        progress_image = cv2.imread(str(fixtures_dir / "progress.png"))

        # Test sequence: PROGRESS -> FAIL -> PROGRESS -> FAIL
        test_sequence = [progress_image, fail_image, progress_image, fail_image]

        # Test with old implementation
        old_state_machine = UpgradeStateMachine(max_attempts=10)
        old_results = []
        for image in test_sequence:
            fail_count, stop_reason = old_state_machine.process_frame(image)
            old_results.append((fail_count, stop_reason))

        # Test with new implementation
        detector = ProgressBarStateDetector()
        new_monitor = UpgradeAttemptMonitor(detector, max_attempts=10)
        new_results = []
        for image in test_sequence:
            new_monitor.process_frame(image)
            new_results.append((new_monitor.fail_count, new_monitor.stop_reason))
        ...
        # Verify final state
        assert old_results[-1][0] == 2  # 2 fails
        assert old_results[-1][1] is None  # No stop condition yet
