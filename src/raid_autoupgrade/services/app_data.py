...
class AppData:
    ...
    The root directory defaults to C:\\ProgramData\\RaidAutoupgrade but can be overridden
    via RAID_AUTOUPGRADE_DATA_DIR environment variable or explicit root_dir parameter.
    """

    DEFAULT_ROOT = Path(os.getenv("PROGRAMDATA", "C:\\ProgramData")) / "RaidAutoupgrade"
    ...
    def _get_configured_root(self) -> Path:
        ...
        Resolution order:
        1. RAID_AUTOUPGRADE_DATA_DIR environment variable
        2. DEFAULT_ROOT (C:\\ProgramData\\RaidAutoupgrade)

        Returns:
            Path to application data root directory.
        """
        env_data_dir = os.getenv("RAID_AUTOUPGRADE_DATA_DIR")
        if env_data_dir:
    ...
    def get_log_file_path(self) -> Path | None:
        ...
        if self.debug_dir:
            return self.debug_dir / "raid_autoupgrade.log"
        return None
